	///////////////////////////////////////////////////////////////////////////////
	// HLC Custom Stuff  [CiC]red_ned http://cic-gaming.co.uk
	///////////////////////////////////////////////////////////////////////////////

	///////////////////////////////////////////////////////////////////////////////
	// Muzzle Attachments
	///////////////////////////////////////////////////////////////////////////////	
	class	hlc_muzzle_545SUP_AK					{ quality = 2; price = 200; };
	class	hlc_muzzle_762SUP_AK					{ quality = 2; price = 300; };
	class	hlc_muzzle_snds_fal						{ quality = 2; price = 500; };
	class	hlc_muzzle_snds_HK33					{ quality = 2; price = 500; };
	class	hlc_muzzle_snds_G3						{ quality = 2; price = 500; };
	class	hlc_muzzle_snds_M14						{ quality = 2; price = 500; };
	class	hlc_muzzle_556NATO_KAC					{ quality = 2; price = 300; };
	class	hlc_muzzle_300blk_KAC					{ quality = 2; price = 300; };
	class	hlc_muzzle_Agendasix					{ quality = 2; price = 250; };
	class	hlc_muzzle_Tundra						{ quality = 2; price = 250; };
	class	hlc_muzzle_Agendasix10mm				{ quality = 2; price = 250; };
	class	hlc_muzzle_snds_AUG						{ quality = 2; price = 300; };
	class	hlc_muzzle_snds_a6AUG					{ quality = 2; price = 300; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Optic Attachments
	///////////////////////////////////////////////////////////////////////////////
	class 	HLC_Optic_PSO1							{ quality = 1; price = 100; };
	class 	HLC_Optic_1p29							{ quality = 1; price = 150; };
	class 	hlc_optic_kobra							{ quality = 1; price = 75; };
	class 	hlc_optic_PVS4FAL						{ quality = 1; price = 500; };
	class 	hlc_optic_suit							{ quality = 1; price = 100; };
	class 	HLC_Optic_ZFSG1							{ quality = 1; price = 100; };
	class 	hlc_optic_accupoint_g3					{ quality = 1; price = 100; };
	class 	hlc_optic_artel_m14						{ quality = 1; price = 100; };
	class 	hlc_optic_LRT_m14						{ quality = 1; price = 200; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Ammunition
	///////////////////////////////////////////////////////////////////////////////
	class   hlc_200rnd_556x45_T_SAW 				{ quality = 1; price = 100;	};	
	class 	hlc_200rnd_556x45_B_SAW					{ quality = 1; price = 100; };
	class	hlc_100Rnd_762x51_B_M60E4				{ quality = 1; price = 100; };
	class	hlc_100Rnd_762x51_T_M60E4				{ quality = 1; price = 100; };	
	class	hlc_100Rnd_762x51_M_M60E4				{ quality = 1; price = 100; };
	class   hlc_5rnd_300WM_SBT_AWM                  { quality = 1; price = 120;};
	class   hlc_5rnd_300WM_AP_AWM                   { quality = 1; price = 120;};
	class   hlc_5rnd_300WM_BTSP_AWM                 { quality = 1; price = 120;};
	class   hlc_5rnd_300WM_mk248_AWM                { quality = 1; price = 120;};
	class   hlc_5rnd_300WM_FMJ_AWM                  { quality = 1; price = 120;};
	class	hlc_30Rnd_545x39_B_AK					{ quality = 1; price = 30; };
	class	hlc_30Rnd_545x39_T_AK					{ quality = 1; price = 30; };
	class	hlc_30Rnd_545x39_EP_AK					{ quality = 1; price = 30; };
	class	hlc_30Rnd_545x39_S_AK					{ quality = 1; price = 30; };				
	class	hlc_45Rnd_545x39_t_rpk					{ quality = 1; price = 30; };	
	class	hlc_30Rnd_762x39_b_ak					{ quality = 1; price = 30; };
	class	hlc_30Rnd_762x39_t_ak					{ quality = 1; price = 30; };
	class	hlc_45Rnd_762x39_t_rpk					{ quality = 1; price = 30; };
	class	hlc_45Rnd_762x39_m_rpk					{ quality = 1; price = 30; };
	class	hlc_75Rnd_762x39_m_rpk					{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_B_fal					{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_t_fal					{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_S_fal					{ quality = 1; price = 30; };
	class	hlc_50rnd_762x51_M_FAL					{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_B_G3					{ quality = 1; price = 30; };
	class	hlc_20rnd_762x51_T_G3					{ quality = 1; price = 30; };
	class	hlc_50rnd_762x51_M_G3					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_b_HK33					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_t_HK33					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_epr_HK33				{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_sost_HK33				{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_B_M14					{ quality = 1; price = 30; };
	class	hlc_20rnd_762x51_T_M14					{ quality = 1; price = 30; };
	class	hlc_50rnd_762x51_M_M14					{ quality = 1; price = 30; };
	class	29rnd_300BLK_STANAG						{ quality = 1; price = 30; };
	class	29rnd_300BLK_STANAG_T					{ quality = 1; price = 30; };
	class	29rnd_300BLK_STANAG_S					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_EPR					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_SOST					{ quality = 1; price = 30; };
	class	hlc_30rnd_556x45_SPR					{ quality = 1; price = 30; };
	class	hlc_30Rnd_9x19_B_MP5					{ quality = 1; price = 30; };
	class	hlc_30Rnd_9x19_GD_MP5					{ quality = 1; price = 30; };
	class	hlc_30Rnd_9x19_SD_MP5					{ quality = 1; price = 30; };
	class	hlc_30Rnd_10mm_B_MP5					{ quality = 1; price = 30; };
	class	hlc_30Rnd_10mm_JHP_MP5					{ quality = 1; price = 30; };
	class	hlc_30Rnd_556x45_B_AUG					{ quality = 1; price = 30; };
	class	hlc_30Rnd_556x45_SOST_AUG				{ quality = 1; price = 30; };
	class	hlc_30Rnd_556x45_SPR_AUG				{ quality = 1; price = 30; };
	class	hlc_30Rnd_556x45_T_AUG					{ quality = 1; price = 30; };
	class	hlc_40Rnd_556x45_B_AUG					{ quality = 1; price = 30; };
	class	hlc_40Rnd_556x45_SOST_AUG				{ quality = 1; price = 30; };
	class	hlc_40Rnd_556x45_SPR_AUG				{ quality = 1; price = 30; };
	class	hlc_25Rnd_9x19mm_M882_AUG				{ quality = 1; price = 30; };
	class	hlc_25Rnd_9x19mm_JHP_AUG				{ quality = 1; price = 30; };
	class	hlc_25Rnd_9x19mm_subsonic_AUG			{ quality = 1; price = 30; };
	class	hlc_10rnd_12g_buck_S12					{ quality = 1; price = 30; };
	class	hlc_10rnd_12g_slug_S12					{ quality = 1; price = 30; };
	class	hlc_VOG25_AK							{ quality = 1; price = 30; };
	class	hlc_GRD_White							{ quality = 1; price = 30; };
	class	hlc_20Rnd_762x51_mk316_fal				{ quality = 1; price = 50; };
	class	hlc_20Rnd_762x51_mk316_G3				{ quality = 1; price = 50; };
	class	hlc_20Rnd_762x51_mk316_M14				{ quality = 1; price = 50; };
	

	///////////////////////////////////////////////////////////////////////////////
	// Sub Machine Guns
	///////////////////////////////////////////////////////////////////////////////
	class 	hlc_smg_mp5k	 						{ quality = 1; price = 200; }; 	
	class	hlc_smg_mp5k_PDW						{ quality = 1; price = 400; };
	class	hlc_smg_mp5a2							{ quality = 1; price = 450; };
	class	hlc_smg_mp5a3							{ quality = 1; price = 450; };
	class	hlc_smg_mp5a4							{ quality = 1; price = 450; };
	class	hlc_smg_mp5n							{ quality = 1; price = 450; };
	class	hlc_smg_mp510							{ quality = 1; price = 550; };
	class	hlc_smg_mp5sd5							{ quality = 1; price = 350; };
	class	hlc_smg_mp5sd6							{ quality = 1; price = 350; };
	class	hlc_smg_9mmar							{ quality = 1; price = 550; };
	class	hlc_rifle_augpara						{ quality = 1; price = 450; };
	class	hlc_rifle_auga2para						{ quality = 1; price = 450; };

	///////////////////////////////////////////////////////////////////////////////
	// Light Machine Guns
	///////////////////////////////////////////////////////////////////////////////	
	class	hlc_rifle_RPK12							{ quality = 3; price = 800; };
	class	hlc_rifle_rpk							{ quality = 3; price = 800; };
	class	hlc_lmg_m60								{ quality = 3; price = 800; };
	class	hlc_lmg_M60E4							{ quality = 3; price = 800; };
	class	hlc_m249_pip4							{ quality = 1; price = 900; };
	class	hlc_lmg_minimi_railed					{ quality = 1; price = 900; };
	
	///////////////////////////////////////////////////////////////////////////////
	// Assault Rifles
	///////////////////////////////////////////////////////////////////////////////
	class	hlc_rifle_ak12							{ quality = 2; price = 400; };
	class	hlc_rifle_ak47							{ quality = 3; price = 500; };
	class	hlc_rifle_aek971						{ quality = 3; price = 500; };
	class	hlc_rifle_aks74							{ quality = 3; price = 400; };
	class	hlc_rifle_aks74_GL						{ quality = 3; price = 600; };
	class	hlc_rifle_aks74u						{ quality = 3; price = 400; };
	class	hlc_rifle_ak74							{ quality = 3; price = 450; };
	class	hlc_rifle_akm							{ quality = 3; price = 500; };
	class	hlc_rifle_akmgl							{ quality = 3; price = 700; };
	class	hlc_rifle_l1a1slr						{ quality = 3; price = 500; };
	class	hlc_rifle_SLR							{ quality = 3; price = 500; };
	class	hlc_rifle_STG58F						{ quality = 3; price = 1000; };
	class	hlc_rifle_FAL5061						{ quality = 3; price = 800; };
	class	hlc_rifle_c1A1							{ quality = 3; price = 500; };
	class	hlc_rifle_LAR							{ quality = 3; price = 500; };
	class	hlc_rifle_SLRchopmod					{ quality = 3; price = 400; };
	class	hlc_rifle_falosw						{ quality = 3; price = 1000; };
	class	hlc_rifle_osw_GL						{ quality = 3; price = 1200; };
	class	hlc_rifle_g3sg1							{ quality = 3; price = 900; };
	class	hlc_rifle_g3a3							{ quality = 3; price = 900; };
	class	hlc_rifle_g3ka4							{ quality = 3; price = 1000; };
	class	HLC_Rifle_g3ka4_GL						{ quality = 3; price = 1200; };
	class	hlc_rifle_hk51							{ quality = 3; price = 800; };
	class	hlc_rifle_hk53							{ quality = 3; price = 800; };
	class	hlc_rifle_RU556							{ quality = 3; price = 750; };
	class	hlc_rifle_RU5562						{ quality = 3; price = 750; };
	class	hlc_rifle_Colt727						{ quality = 3; price = 750; };
	class	hlc_rifle_Colt727_GL					{ quality = 3; price = 950; };
	class	hlc_rifle_bcmjack						{ quality = 3; price = 850; };
	class	hlc_rifle_Bushmaster300					{ quality = 3; price = 750; };
	class	hlc_rifle_vendimus						{ quality = 3; price = 750; };
	class	hlc_rifle_SAMR							{ quality = 3; price = 850; };
	class	hlc_rifle_samr2							{ quality = 3; price = 850; };
	class	hlc_rifle_honeybadger					{ quality = 3; price = 750; };
	class	hlc_rifle_aug							{ quality = 3; price = 650; };
	class	hlc_rifle_auga1carb						{ quality = 3; price = 650; };
	class	hlc_rifle_aughbar						{ quality = 3; price = 750; };
	class	hlc_rifle_auga2							{ quality = 3; price = 650; };
	class	hlc_rifle_auga2carb						{ quality = 3; price = 650; };
	class	hlc_rifle_auga2lsw						{ quality = 3; price = 650; };
	class	hlc_rifle_augsr							{ quality = 3; price = 650; };
	class	hlc_rifle_augsrcarb						{ quality = 3; price = 650; };
	class	hlc_rifle_augsrhbar						{ quality = 3; price = 750; };
	class	hlc_rifle_auga3							{ quality = 3; price = 650; };
	class	hlc_rifle_auga3_GL  					{ quality = 3; price = 850; };
	class	hlc_rifle_CQBR					  		{ quality = 3; price = 750; };
	class	hlc_rifle_m4m203				 		{ quality = 3; price = 1000; };
	class	hlc_rifle_M4				        	{ quality = 3; price = 800; };
	class   hlc_rifle_m14sopmod						{ quality = 1; price = 1050; };
	

	///////////////////////////////////////////////////////////////////////////////
	// Sniper Rifles
	///////////////////////////////////////////////////////////////////////////////	
	class	hlc_rifle_M21							{ quality = 3; price = 800; };
	class	hlc_rifle_psg1							{ quality = 3; price = 1000; };	
	class 	hlc_rifle_awcovert						{ quality = 1; price = 2000;};
	class 	hlc_rifle_awmagnum						{ quality = 1 ; price = 1500;};
	class	hlc_rifle_awMagnum_OD_ghillie			{ quality = 1 ; price = 2000;};